...
export function StorefrontShell({ children }: { children: React.ReactNode }) {
  const jsonLd = {
    ...
    logo: "https://khaobetter.shop/assets/kb-logo.png",
    sameAs: []
  };
...
export function OurStoryContent() {
  ...
  return <main><section className="story-hero"><div className="container"><div className="eyebrow">Our story</div><h1>We Got Tired of Choosing Between Tasty and Better</h1><p>Khao Better is being built around a simple idea: everyday snacks should feel enjoyable without relying on the same old fried options.</p></div></section><section className="section"><div className="container two-col"><div><div className="eyebrow">The snack moment</div><h2>A Brand Philosophy for Better Everyday Snacks</h2><p>Khao Better is a pre-launch roasted makhana brand for people who want a crunchy Indian snack that fits into ordinary routines: a work desk, a study table, a commute bag or a relaxed evening at home.</p><p>The first product is Classic Roasted Makhana, a 25 g roasted fox nuts pack priced at Rs. 59. It is currently out of stock while the brand prepares for launch, so this site stays focused on discovery and waitlist interest.</p></div><div className="promise-card"><h2>What guides Khao Better</h2>{principles.map((promise) => <div key={promise[0]}><b>*</b><span><strong>{promise[0]}</strong><small>{promise[1]}</small></span></div>)}</div></div></section><section className="section alt"><div className="container"><div className="section-head"><div><div className="eyebrow">Why makhana</div><h2>Built Around a Familiar Indian Crunch</h2></div><Link className="secondary" href="/why-makhana">Understand makhana</Link></div><div className="info-grid"><article><span className="icon">Office</span><h3>For office workers</h3><p>A compact roasted snack for desk drawers and afternoon breaks.</p></article><article><span className="icon">Students</span><h3>For students</h3><p>A simple pack format for study sessions and bag-friendly snacking.</p></article><article><span className="icon">Daily</span><h3>For everyday snackers</h3><p>A familiar Indian snack made easy to understand before launch.</p></article></div><div className="hero-actions centered-button"><Link className="primary" href="/shop">See Khao Better roasted makhana flavours</Link><WaitlistButton product={catalog[0]} className="secondary">Join the Classic waitlist</WaitlistButton></div></div></section></main>;
}

function Footer() {
  const cols = [
    ["Flavours", ["/shop", "Shop"], ["/makhana-classic-roasted", "Classic Roasted"]],
    ["Learn", ["/why-makhana", "Why Makhana"], ["/blog", "Blog"], ["/blog/makhana-nutrition-facts", "Nutrition"], ["/our-story", "Our Story"]],
    ["Launch", ["/shop", "Coming Soon Flavours"], ["/makhana-classic-roasted", "Join Classic Waitlist"]]
  ];
  return <footer><div className="container footer-grid"><div><img src="/assets/kb-logo.png" alt="Khao Better" /><p>A better snack for everyday. Khao Better is preparing roasted makhana for launch.</p></div>{cols.map((col) => <div key={col[0] as string}><h3>{col[0]}</h3>{col.slice(1).map((item) => Array.isArray(item) ? <Link href={item[0]} key={item[1]}>{item[1]}</Link> : null)}</div>)}<div><h3>Join the waitlist</h3><p>We'll let you know when Khao Better is ready.</p><div className="subscribe"><WaitlistButton product={catalog[0]}>Join the waitlist</WaitlistButton></div></div></div><div className="container footer-bottom"><span>© 2026 Khao Better</span><span><Link href="/why-makhana">Why Makhana</Link> · <Link href="/our-story">Our Story</Link></span></div></footer>;
}
